...
public class RentalService(AppDataContext context, IdGenerator idGenerator, IPenaltyCalculator penaltyCalculator)
{
    public Rental RentEquipment(int userId, int equipmentId, int days, DateTime? rentDate = null)
    {
        ...
        var activeRentalsCount = context.Rentals.Count(r => r.User.Id == userId && !r.IsReturned);
    ...
    public decimal ReturnEquipment(int rentalId, DateTime returnDate)
    {
        var rental = context.Rentals.FirstOrDefault(r => r.Id == rentalId) 
                     ?? throw new BusinessException($"Rental with id {rentalId} was not found.");
        ...
        var penalty = penaltyCalculator.Calculate(rental.DueDate, returnDate);
    ...
    {
        return context.Rentals.Where(r => r.User.Id == userId && !r.IsReturned).ToList();
    }

    public List<Rental> GetOverdueRentals()
    {
        return context.Rentals.Where(r => r is { IsReturned: false, IsOverdue: true }).ToList();
    }
    ...
    {
        return context.EquipmentItems.FirstOrDefault(e => e.Id == equipmentId) 
               ?? throw new BusinessException($"Equipment with id {equipmentId} was not found.");
    }
}